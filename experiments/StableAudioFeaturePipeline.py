...
class StableAudioFeaturePipeline(DiffusionPipeline):
    r"""
    ...
    def prepare_latents(
        ...
        if initial_audio_waveforms is not None:
            ...
            # crop or pad
            print('INITIAL AUDIO WAVEFORMS: ', initial_audio_waveforms.shape)
            audio_length = initial_audio_waveforms.shape[-1]
            print('COMPARISON: ', audio_length, audio_vae_length)
    ...
    def __call__(
        ...
        # Added argument to handle features
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Addition for feature collection
                ...
                noise_pred = self.transformer(
                    ...
                    return_dict=False,
                    collect_features=collect_features # Addition for feature collection
                )[0]

                # see ~/src/FluxFeaturePipeline.py, line 949
                if collect_features and run_feats:
    ...
    # A version of __call__() that only runs the model and returns the features
    @torch.no_grad()
    @replace_example_docstring(EXAMPLE_DOC_STRING)
    def extract_feats(
        self,
        ...
        noise_scheduler_copy: Optional[EDMDPMSolverMultistepScheduler] = None,
    ):
        r"""
        The call function to the pipeline for generation.
        ...
        self.check_inputs(
            ...
            attention_mask,
            negative_attention_mask,
            initial_audio_waveforms, # input audio goes here
            initial_audio_sampling_rate,
        )
        ...
        # print('Scheduler: ', self.scheduler.timesteps)
        timesteps = torch.tensor([1000]).to(device).long() #self.scheduler.timesteps[0]
        ...
        # TODO: CHECK FEATURE SLICING WRT MODEL ARCHITECTURE
        for j in range(len(self.transformer.features)):
            # print(self.transformer.features[j]['hidden'].size())
            if feats is None:
                feats = self.transformer.features[j]['hidden'][:, 1:, :].to(self.feature_dtype)
            else:
                feats = torch.cat((feats, self.transformer.features[j]['hidden'][:, 1:, :].to(self.feature_dtype)), dim=0)
        if time_feats is None:
            time_feats = self.transformer.time_emb.to(self.feature_dtype)
        else:
            time_feats = torch.cat((time_feats, self.transformer.time_emb.to(self.feature_dtype)), dim=0)
