...
def musicgen(
    ...
    # # Processor and model (MusicGen Melody)
    # processor = AutoProcessor.from_pretrained("facebook/musicgen-medium", low_cpu_mem_usage=True)
    # print('processor complete')
    # model = MusicgenMelodyForConditionalGeneration.from_pretrained("facebook/musicgen-medium")
    # print('model complete')
...
if __name__ == '__main__':
    try:
        text_prompt: str | list[str] | list[list[str]] = sys.argv[1] 
    except:
        ValueError('No text prompt or invalid text prompt provided')
    
    names = musicgen(text_prompt= text_prompt)
