...
def padding(kernel_size, dilation=1) -> int:
...
class ResnetBlock1(nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.conv1 = nn.ModuleList([
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=dilation[0], padding=nn.get_padding(kernel_size, dilation[0]))),
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=dilation[1], padding=nn.get_padding(kernel_size, dilation[1]))),
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=dilation[2], padding=nn.get_padding(kernel_size, dilation[2])))
        ])
        #self.conv1.apply(init_weights) mean=0, std=0.01

        self.conv2 = nn.ModuleList([
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=1, padding=nn.get_padding(kernel_size, 1))),
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=1, padding=nn.get_padding(kernel_size, 1))),
            weight_norm(nn.Conv1d(channels, channels, kernel_size, 1, dilation=1, padding=nn.get_padding(kernel_size, 1)))
        ])
        #self.conv2.apply(init_weights) mean=0, std=0.01

    def forward(self, x):
        for c1, c2 in zip(self.conv1, self.conv2):
            xt = F.leaky_relu(x, LEAKY_SLOPE)
            xt = c1(xt)
            xt = F.leaky_relu(x, LEAKY_SLOPE) 
            xt = c2(xt)
            x += xt
...
class Generator(nn.Module):
    def __init__(self, h):
        ...
        resblock = ResnetBlock1
        
        self.ups = nn.ModuleList()
        for i, (u, k) in enumerate(zip(h.upsample_rates, h.upsample_kernel_sizes)):
            self.ups.append(weight_norm(
                nn.ConvTranspose1d(h.upsample_initial_channel//(2**i), h.upsmaple_initial_channel//(2**(i+1)), k, u, padding=(k-u)//2)))
